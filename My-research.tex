...
\hypertarget{hosting-bookdown-on-github}{%
\chapter{Hosting Bookdown on Github}\label{hosting-bookdown-on-github}}
...
\tightlist
\item
  From the \href{https://bookdown.org/yihui/bookdown/get-started.html}{Get started} page download the Zip file, then unzip it locally.
\item
...
You should now have a folder called \texttt{bookdown-demo-master} which contains exemple chapters, an index and files needed for the Bookdown.

\textbf{In the Bookdown-demo-master folder}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Create a folder called \texttt{docs} and leave it empty
\item
  Look for a file called \texttt{\_bookdown.yml}
\item
  Open the file
\item
  Add \texttt{output\_dir:\ "docs"}
\end{enumerate}
...
\textbf{Create a repository online}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Create a repository on Github where you will host your Bookdown (exemple \texttt{test\_book})
\item
  Copy the repository link generated by Github
\end{enumerate}

\begin{center}\includegraphics[width=1\linewidth]{github_repo_link} \end{center}

\textbf{Clone the newly created repository on your computer}
...
\item
  To clone your repository use the command \texttt{git\ clone}
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{git}\NormalTok{ clone https://github.com/karinevilleneuve/test\_book.git}
\end{Highlighting}
\end{Shaded}

In this exemple, the Github link would be replaced by the link you copied earlier.

\hypertarget{from-bookdown-to-github}{%
\section{From Bookdown to Github}\label{from-bookdown-to-github}}

Copy the entire content of the Bookdown folder into the new folder create by cloning the repository. In this case this folder is called \texttt{test\_book}

Now you can start modifying the content of your book by opening each chapter.

What is now very \textbf{important} is that, once you \texttt{knit} your R Markdown, the ouput HTML will be created into the folder \texttt{docs} (the one we created earlier). This is important because Github will use the files from the folder \texttt{docs} to generate the Github page.

\hypertarget{knit-and-upload-to-github}{%
\section{Knit and upload to github}\label{knit-and-upload-to-github}}

\includegraphics[width=0.35\linewidth,style="float:right; padding:10px"]{build_book}
...
\tightlist
\item
  Open the file with the extension \texttt{.Rproj}
\item
  Click on \texttt{Buil\ Book} under the \texttt{Build} tab on the right
\end{enumerate}

If you navigate to the folder \texttt{docs} you should now see that it contains a HTML copy of every chapter

\textbf{Update your online repository}
...
\tightlist
\item
  Go to your \textbf{online Github repository}, all the files and folders from your local folder should be there
\item
  Go to \texttt{Settings}
\item
  Scroll down to the section \texttt{Github\ Pages}
\item
  Click on button \texttt{None} and from the dropdown menu select \texttt{Master}
\item
  Click the \texttt{/(root)} button and from the dropdown menu select \texttt{/(docs)}
\item
  Click \texttt{Save}
\item
  The page will refresh, and now if you scroll back down to the section \texttt{Github\ Pages} you will see the URL of your Book
\end{enumerate}

\emph{Exemple}

\begin{center}\includegraphics[width=1\linewidth]{github_pages} \end{center}

Voilà

\hypertarget{texas-protocol}{%
\chapter{Texas Protocol}\label{texas-protocol}}

\hypertarget{unzip-the-gz-files}{%
\section{Unzip the gz files}\label{unzip-the-gz-files}}
...
\section{Interleaving}\label{interleaving}}
...
\emph{python /directory/to/script/nameofscript.py file\_R1 file\_R2 \textgreater{} name\_of\_combined\_file.fastq}
...
Move all interleaved fastq files into a single folder
...
\textbf{Bash script for multiple files}
...
This will generate HTML files that you need to transfer to your local computer in order to view them in a chrome web page
...
\begin{itemize}
\item
  This generates two HTML file (one for the combined\_trimmed and one for the combined)
\item
...
\textbf{Running with Bash for multiple files}
...
\hypertarget{assembling}{%
\section{Assembling}\label{assembling}}

There exists multiple assembling programs\ldots{} it can be hard to chose the best one. I suggest trying a few and seeing what gives you the best results (number of sequence and average length of sequences)

\hypertarget{idba}{%
\subsection{IDBA}\label{idba}}

\href{https://github.com/loneknightpy/idba}{Github}

Generate a folder called \texttt{assembly}and the file we are interested in is called \texttt{contig.fa}

\textbf{Bash script using VI document}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Create a VI document named \texttt{runidba.sh}containing the following script. To save and quit (\texttt{ESC}+ \texttt{:qw})
\end{enumerate}
...
\tightlist
\item
  Make the \texttt{runidba.sh} to make it executable
\end{enumerate}
...
\tightlist
\item
  Run the script with \texttt{nohup} and \texttt{\&} (won't be affected by connectivity problem)
\end{enumerate}
...
\emph{View the tasks running on the server by using \texttt{jobs}or \texttt{htop}}

\emph{\texttt{free}allows you to see the available memory}
...
\hypertarget{megahit}{%
\subsection{Megahit}\label{megahit}}

\href{https://github.com/voutcn/megahit}{Github} \href{https://www.ncbi.nlm.nih.gov/pubmed/25609793}{NCBI}
...
\tightlist
\item
  Inside the folder containing your \texttt{fastq} files create a VI doc. called \texttt{megahit.sh} with the following script and make it executable
\end{enumerate}
...
\subsection{metaSPAdes}\label{metaspades}}
...
\section{POST ASSEMBLY STATS}\label{post-assembly-stats}}
...
\setcounter{enumi}{1}
\item
  Remove the first row of the document lenghts (open the file in a text editor app like \texttt{nano} or \texttt{vi})
\item
...
\tightlist
\item
  Transfer the file to your local computer to view it and confirm how many sequences are greater than 2000
\end{enumerate}
...
\tightlist
\item
  Run the perl script length+GC.pl
\end{enumerate}
...
\tightlist
\item
  View the GC content \texttt{less\ contig\_GC.txt}
\end{enumerate}
...
Count how many you have above 2000 kb
...
\href{http://bio-bwa.sourceforge.net/bwa.shtml}{BWA Reference Manual} and Wrapper \href{https://github.com/imrambo/genome_mapping}{Github}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Put the fasta file with the long sequence as well as the trimmed-combined fastq file of each sample in the same folder
\item
...
\tightlist
\item
  Align the fasta file against the \texttt{combined\_trimmed.fastq}file
\end{enumerate}
...
\tightlist
\item
  Make \texttt{mapping.sh} script executable \texttt{chmod\ +x\ mapping.sh} and run it script using \texttt{nohup\ ./} and \texttt{\&}
\end{itemize}
...
\tightlist
\item
  Convert \texttt{.sam} file to \texttt{.bam} file with \href{http://www.metagenomics.wiki/tools/samtools}{samtools}
\end{enumerate}
...
\tightlist
\item
  Make script executable \texttt{chmod\ +x\ samtools.sh} and run the script in the background using \texttt{nohup\ ./} and \texttt{\&}
\end{itemize}
...
\tightlist
\item
  Sort the bam file (required in order to use the script to generate a depth file)
\end{enumerate}
...
\tightlist
\item
  Make script executable \texttt{chmod\ +x\ sort.sh} and run script using \texttt{nohup\ ./} and \texttt{\&}
\end{itemize}
...
Open ouput depth file with \texttt{less\ depth.txt}
...
\section{BIN QUALITY}\label{bin-quality}}
...
\section{BIN CLEANING}\label{bin-cleaning}}
...
\subsection{MM Genome}\label{mm-genome}}
...
\hypertarget{vizbin}{%
\subsection{Vizbin}\label{vizbin}}
...
\section{BIN TAXONOMY}\label{bin-taxonomy}}

\hypertarget{barrnap}{%
\subsection{Barrnap}\label{barrnap}}
...
\hypertarget{gtdbtk}{%
\subsection{GTDBTK}\label{gtdbtk}}
...
\subsection{OneCodeX}\label{onecodex}}

\hypertarget{phylogenetic-tree}{%
\section{PHYLOGENETIC TREE}\label{phylogenetic-tree}}

\textbf{Download reference genomes}
...
\subsection{Phylosift}\label{phylosift}}
...
\subsection{Geneious}\label{geneious}}
...
\section{METABOLIC PATHWAY}\label{metabolic-pathway}}

\textbf{Convert bins to protein (.faa)}
...
\textbf{Hydrogrenase Database sequence lenght}
